...
# RK3399 boards are commonly used as low-memory build hosts.  Keep the
# default conservative; users with larger hosts can set WORLDJOBS/KERNJOBS.
WORLDJOBS=${WORLDJOBS:-2}
KERNJOBS=${KERNJOBS:-2}
...
if [ -z "${SOM_RK3399_FIRMWARE_ROOT}" ]; then
    SOM_RK3399_FIRMWARE_ROOT=`host_first_existing_dir \
        ...
        "/usr/share/u-boot/som-rk3399" \
        "${TOPDIR}/external/som-rk3399/u-boot" \
        "${TOPDIR}/external/som-rk3399/artifacts" \
...
strategy_add $PHASE_CHECK som_rk3399_check_firmware

som_rk3399_ensure_dts ( ) {
    SOM_RK3399_DEST_DTS=${SOM_RK3399_DTB_DESTDIR}/${SOM_RK3399_DTB_NAME}

    if [ ! -f "${SOM_RK3399_DEST_DTS}" ]; then
        if [ ! -f "${SOM_RK3399_DTB_SOURCE}" ]; then
            echo "Missing SOM-RK3399 DTS in both source trees."
            echo "Expected one of:"
            echo "    ${SOM_RK3399_DEST_DTS}"
        ...
        cp "${SOM_RK3399_DTB_SOURCE}" "${SOM_RK3399_DEST_DTS}" || exit 1
    fi
...
strategy_add $PHASE_CHECK som_rk3399_ensure_dts

som_rk3399_install_dtb ( ) (
    SOM_RK3399_DTS_PATH=${SOM_RK3399_DTB_DESTDIR}/${SOM_RK3399_DTB_NAME}
    SOM_RK3399_TMPDIR=${WORKDIR}/som-rk3399-dtb
    SOM_RK3399_TMPDTB=${SOM_RK3399_TMPDIR}/${SOM_RK3399_DTB_BASENAME}.dtb

    mkdir -p "${SOM_RK3399_TMPDIR}" || exit 1
    rm -f "${SOM_RK3399_TMPDTB}"

    echo "${FREEBSD_SRC}/sys/tools/fdt/make_dtb.sh ${FREEBSD_SRC}/sys ${SOM_RK3399_DTS_PATH} ${SOM_RK3399_TMPDIR}" | (cd "${FREEBSD_SRC}"; ${FREEBSD_MAKE} TARGET_ARCH=$TARGET_ARCH buildenv > /dev/null)
    if [ ! -f "${SOM_RK3399_TMPDTB}" ]; then
        echo "Failed to build ${SOM_RK3399_DTB_BASENAME}.dtb from:"
        echo "    ${SOM_RK3399_DTS_PATH}"
        exit 1
    fi
    cp "${SOM_RK3399_TMPDTB}" EFI/BOOT/rk3399-som-rk3399.dtb || exit 1
)

som_rk3399_partition_image ( ) {
...
strategy_add $PHASE_PARTITION_LWW som_rk3399_partition_image

som_rk3399_make_rootfs_image ( ) {
...
strategy_add $PHASE_POST_UNMOUNT som_rk3399_compose_image
...
strategy_add $PHASE_BOOT_INSTALL som_rk3399_install_dtb
